...
class G1Env(DirectRLEnv):
    ...
    def _init_buffers(self):
        ...
        self.phase_freq = 0.5  # frequency of gait cycle in Hz
    ...
    def _post_physics_step_callback(self):
        ...
        """
        # Update gait phase for contact reward
        self.leg_phase += self.sim.cfg.dt * self.phase_freq
        self.leg_phase = self.leg_phase % 1.0  # Keep phase in [0, 1]
        # Make legs alternate: left foot phase offset by 0.5
        self.leg_phase[:, 1] = (self.leg_phase[:, 1] + 0.5) % 1.0
    ...
    def _reward_feet_swing_height(self):
        """
        Encourage proper swing leg height around 8cm during swing phase.
        Penalizes deviation from target height (0.08m) when feet are not in contact with ground.
        This promotes natural stepping motion with appropriate ground clearance.
        """
        # Detect which feet are in contact (force > 1N)
        contact = torch.norm(self.contact_forces[:, self.feet_indices, :3], dim=2) > 1.0  # [num_envs, 2]
        
        # Calculate height error from target (0.08m = 8cm) only during swing phase
        pos_error = torch.square(self.feet_pos[:, :, 2] - 0.15) * ~contact  # [num_envs, 2]
        
        # Sum error over both feet
        return torch.sum(pos_error, dim=1)  # [num_envs]
